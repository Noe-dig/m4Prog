...
drop table if exists `telefoonGesprek`;
create table `klant` (
    ...
    unique index `id__unique` (`klantid` asc) visible
);
...
create table `telefoonGesprek` (
	...
	unique index `id__unique` (`gesprekid` asc) visible
) 
...
select * from telefoonGesprek where klantid = 1;
-- naam, leeftijd, telefoonnummer zijn dubbel